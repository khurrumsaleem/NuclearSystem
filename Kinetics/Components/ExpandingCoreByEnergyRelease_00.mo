...
model ExpandingCoreByEnergyRelease_00
  ...
  /* ---------------------------------------------
              switches
        --------------------------------------------- */
  parameter NuclearSystem.Types.Switches.switchHowToDetVar switchDetermine_massCore = NuclearSystem.Types.Switches.switchHowToDetVar.param "switch how to determine massCore" annotation(
    Dialog(group = "switch"),
  ...
  Modelica.Blocks.Interfaces.RealInput u_massCore(unit="kg") if switchDetermine_massCore == NuclearSystem.Types.Switches.switchHowToDetVar.viaRealInput "massCore input, valid only when switchDetermine_massCore==viaRealInput" annotation(
    Placement(transformation(origin = {-120, 0}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-110, 0}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Interfaces.RealInput u_Vol0(unit="m3") if switchDetermine_Vol0 == NuclearSystem.Types.Switches.switchHowToDetVar.viaRealInput "massCore input, valid only when switchDetermine_massCore==viaRealInput" annotation(
    Placement(transformation(origin = {-120, -60}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-110, -60}, extent = {{-10, -10}, {10, 10}})));
...
/******************************
  ******************************/
//---
  if switchDetermine_massCore == NuclearSystem.Types.Switches.switchHowToDetVar.param then
    massCore = massCore_par;
  elseif switchDetermine_massCore == NuclearSystem.Types.Switches.switchHowToDetVar.viaRealInput then
    massCore = u_massCore;
  end if;
//---
  E = u_E;

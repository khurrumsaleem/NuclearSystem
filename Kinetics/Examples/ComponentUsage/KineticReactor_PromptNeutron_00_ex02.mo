...
model KineticReactor_PromptNeutron_00_ex02
  ...
  Components.KineticReactor_PromptNeutron_00 PtRctr(use_HeatTransfer = false)  annotation(
    Placement(transformation(origin = {81, 16}, extent = {{-15, -15}, {15, 15}})));
  ...
  Modelica.Blocks.Interaction.Show.RealValue realValue_pwr(significantDigits = 4) annotation(
    Placement(transformation(origin = {141, 7}, extent = {{-17, -9}, {17, 9}})));
  ...
  Modelica.Blocks.Math.Sum sumRho(nin = 2) annotation(
    Placement(transformation(origin = {50, 16}, extent = {{-6, -6}, {6, 6}})));
  ...
  Modelica.Blocks.Math.Gain sig2act(k = 0.0001) annotation(
    Placement(transformation(origin = {-11, 16}, extent = {{-7, -7}, {7, 7}})));
  Utilities.rho2keff rho2keff_act annotation(
    Placement(transformation(origin = {42, -10}, extent = {{-6, -6}, {6, 6}})));
equation
  connect(PtRctr.y_pwrRel0, realValue_pwr.numberPort) annotation(
    Line(points = {{97, 7}, {121, 7}}, color = {0, 0, 127}));
  connect(PtRctr.y_pwr, realValue_pwr1.numberPort) annotation(
    Line(points = {{97, 10}, {115, 10}, {115, 29}, {120, 29}}, color = {0, 0, 127}));
  ...
  connect(delay_sensor.u, PtRctr.y_pwrRel0) annotation(
    Line(points = {{52, -60}, {114, -60}, {114, 7}, {97, 7}}, color = {0, 0, 127}));
  ...
  connect(PtRctr.u_rho, sumRho.y) annotation(
    Line(points = {{64.5, 16}, {57, 16}}, color = {0, 0, 127}));
  connect(actDelay.y, sumRho.u[1]) annotation(
    Line(points = {{25, 16}, {43, 16}}, color = {0, 0, 127}));
  connect(keff2rho.y_rho, sumRho.u[2]) annotation(
    Line(points = {{12, 56}, {34, 56}, {34, 16}, {43, 16}}, color = {0, 0, 127}));
  connect(sig2act.y, actDelay.u) annotation(
    Line(points = {{-3.3, 16}, {2.4, 16}}, color = {0, 0, 127}));
  connect(sumCtrl.y, sig2act.u) annotation(
    Line(points = {{-35, 16}, {-19, 16}}, color = {0, 0, 127}));
  connect(actDelay.y, rho2keff_act.u_rho) annotation(
    Line(points = {{25, 16}, {30, 16}, {30, -10}, {35, -10}}, color = {0, 0, 127}));
  annotation(
